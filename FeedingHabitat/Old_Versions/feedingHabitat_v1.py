...
###############################################################################
    ...
    def __init__(self) :
        ...
        self.cohorts_number     = None
        self.layers_number      = None
        self.root_directory      = None
        self.output_directory   = None
        # First cohort is number 0 and last is number K-1
        self.cohorts_to_compute = None
        self.partial_oxygen_time_axis = False
        
        self.variables_dictionary = {"forage_epi"          : None,
                                     "forage_meso"         : None,
                                     "forage_mmeso"        : None,
                                     "forage_bathy"        : None,
                                     "forage_mbathy"       : None,
        ...
        self.parameters_dictionary = {"eF_list"  : None,
                                      "sigma_0"  : None,
                                      "sigma_K"  : None,
                                      "T_star_1" : None,
                                      "T_star_K" : None,
...
###############################################################################
    ...
    def __readXmlConfigFilepaths__(self, root) :
        ...
        for temp_file in root.find('strfile_t').attrib.values() :
            temperature_filepaths.append(self.root_directory +temp_file)
        
        # OXYGEN ##############################################################
        oxygen_filepaths = []
        for oxy_file in root.find('strfile_oxy').attrib.values() :
            oxygen_filepaths.append(self.root_directory +oxy_file)
        ...
        for forage in ordered_forage :
            forage_filepaths.append(self.root_directory +forage_directory +'Fbiom_'+forage+".nc")
            
        # MASK ################################################################
        mask_filepath = self.root_directory +root.find('str_file_mask').attrib['value']
...
###############################################################################
    ...
    def __loadVariablesFromFilepaths__(self, temperature_filepaths, oxygen_filepaths, forage_filepaths, mask_filepath) :
        ...
        # MASK ################################################################
        self.__setMask__(from_text=mask_filepath)        
        
        # DAY LENGTH ##########################################################
        # TODO : Essayer avec PISCES
        self.daysLength(float_32=True)
        
###############################################################################
###############################################################################

    def __setMask__(self, from_text=None, expend_time=True) :
        # 0 = ground
        # 1 = L1
        # 2 = L2
        # 3 = L3
...
###############################################################################
    ...
    def loadFromXml(self, xml_filepath, partial_cohorts_computation=None) :
        ...
        self.root_directory  = root.find('strdir').attrib['value']
...
###############################################################################
    ...
    def saveHaStepByStep(self, filepath=None, verbose=True) :
        
        path_save = (self.root_directory  + self.output_directory) if filepath is None else filepath
        if verbose :
            print('Files will be saved at : %s' % path_save)
