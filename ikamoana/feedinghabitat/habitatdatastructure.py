...
def equalCoords(
        data_array_1: xr.DataArray, data_array_2: xr.DataArray, verbose: bool = False
        ) -> bool :
    """
...
def compareDims(
        data_array_1: xr.DataArray, data_array_2: xr.DataArray, dim: str,
        head: int = None) :
    """
...
class HabitatDataStructure :
    ...
    def findCoordIndexByValue(
        ...
        for v in val:
            ...
            if verbose :
                print("The %s coordinate closest to %s is index %d, which "
                      "is %s"%(coord, v, index, val_list[index].data))
            val_index_list.append(index)

        return val_index_list

    def findCohortByLength(
            self, length: Union[float, List[float]], verbose: bool = False
            ) -> List[int] :
        """
        ...
        for l in length :
            ...
            if verbose :
                print("The cohort with the length closest to %f is cohort "
                      "number %d whose length is %f"%(l, index, length_list[index]))
            cohort_number_list.append(index)

        return  cohort_number_list

    def findCohortByWeight(
            self, weight: Union[float, List[float]], verbose: bool = False
            ) -> List[int] :
        """
        ...
        for w in weight :
            ...
            if verbose :
                print("The cohort with the weight closest to %f is cohort "
                      "number %d whose weight is %f"%(w, index, weight_list[index]))
            cohort_number_list.append(index)

        return cohort_number_list

    def findLengthByCohort(
            self, cohort: Union[float, List[float]], verbose: bool = False
            ) -> List[int] :
        """
