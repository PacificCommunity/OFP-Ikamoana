...
def _loadVariablesFromFilepaths(root, temperature_filepaths, oxygen_filepaths,
    ...
    variables_dictionary["oxygen_L1"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(oxygen_filepaths[0]))
    variables_dictionary["oxygen_L2"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(oxygen_filepaths[1]))
    variables_dictionary["oxygen_L3"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(oxygen_filepaths[2]))


    # TEMPERATURE #############################################################
    variables_dictionary["temperature_L1"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(temperature_filepaths[0]))
    variables_dictionary["temperature_L2"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(temperature_filepaths[1]))
    variables_dictionary["temperature_L3"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(temperature_filepaths[2]))
    ...
    variables_dictionary["forage_epi"]     = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[0]))
    variables_dictionary["forage_meso"]    = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[1]))
    variables_dictionary["forage_mmeso"]   = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[2]))
    variables_dictionary["forage_bathy"]   = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[3]))
    variables_dictionary["forage_mbathy"]  = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[4]))
    variables_dictionary["forage_hmbathy"] = xr.apply_ufunc(
        nan_add, xr.open_dataarray(forage_filepaths[5]))


    # SST #####################################################################
    variables_dictionary["sst"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(sst_filepath))

    # ZEU #####################################################################
    variables_dictionary["zeu"] = xr.apply_ufunc(
        np.nan_to_num, xr.open_dataarray(zeu_filepath))

    # LENGTHS & WEIGHT ########################################################
    variables_dictionary['cohorts_mean_length'] = np.array(
        [float(x) for x in root.find('length').find(sp_name).text.split()])
    variables_dictionary['cohorts_mean_weight'] = np.array(
...
def loadFromXml(xml_filepath: str,
    ...
    # Variables ###############################################################
    (variables_dictionary,
    global_mask, coords) = _loadVariablesFromFilepaths(root,
                                                      temperature_filepaths,
                                                      oxygen_filepaths,
                                                      forage_filepaths,
                                                      sst_filepath,
